...
1. 사용자가 여행 날짜를 입력합니다.
2. 해당 날짜의 기존 결과 데이터가 있는지 확인합니다.
3. 기존 데이터가 있으면 API 호출을 생략하고 저장된 데이터를 사용합니다.
4. 기존 데이터가 없으면 Gemini API를 이용하여 여행지를 추천받습니다.
5. Gemini의 응답을 JSON 형식으로 파싱합니다.
6. 추천된 도시를 Kakao Local API에 전달하여 맛집을 검색합니다.
7. 원본 JSON 데이터를 `results/` 폴더에 저장합니다.
8. 추천 결과와 맛집 정보를 이용하여 최종 여행 리포트를 생성합니다.
9. 최종 Markdown 리포트를 `results/` 폴더에 저장합니다.
---
...
- 기존 결과 데이터가 있으면 API 호출을 생략하는 결과 캐싱 기능
...
생성된 Markdown 파일은 VSCode 또는 GitHub에서 확인할 수 있습니다.

같은 날짜로 프로그램을 다시 실행하면 기존 결과 데이터를 확인하여
Gemini 및 Kakao API 호출을 생략하고 저장된 데이터를 재사용합니다.
